...
# 1. All channels are candidates
candidate_indexes = list(range(target_conv.out_channels))

# 2. Build candidate group
candidate_group = DG.get_pruning_group(
    target_conv,
    tp.prune_conv_out_channels,
    idxs=candidate_indexes
)

# 3. Calculate Group L2 importance
importance = tp.importance.GroupMagnitudeImportance(p=2)

scores = importance(candidate_group)

print("Importance scores:", scores)
print("Importance scores shape:", scores.shape)

# 4. Select k channels with lowest scores
k = 2

score_indexes = torch.argsort(scores)[:k]

# Convert score positions -> actual channel indexes
pruning_indexes = [
    candidate_indexes[i] for i in score_indexes.tolist()
]

print("Selected pruning indexes:", pruning_indexes)
print("Selected scores:", scores[score_indexes])

# 5. Create a NEW group containing only selected channels
...
print(pruning_group)
print("Group valid:", DG.check_pruning_group(pruning_group))
